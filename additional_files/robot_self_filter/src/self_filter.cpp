...
namespace robot_self_filter
{
  ...
  private:
    void cloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud)
    {
      ...
      if (!tf_buffer_->canTransform("world", cloud->header.frame_id, cloud->header.stamp, rclcpp::Duration::from_seconds(0.01)))  // small timeout
      {
        RCLCPP_INFO(this->get_logger(), "Skipping cloud: TF not available");
