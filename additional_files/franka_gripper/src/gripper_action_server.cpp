...
void GripperActionServer::onExecuteGripperCommand(
  ...
  constexpr double kSamePositionThreshold = 1e-3;
  ...
  auto command = [kTargetWidth, kCurrentWidth, kGoal, this]() {
    if (kTargetWidth >= kCurrentWidth || kTargetWidth == 0.07 || kTargetWidth == 0.0) {
      return gripper_->move(kTargetWidth, default_speed_);
    }
    return gripper_->grasp(kTargetWidth, default_speed_, 30.0,
...
void GripperActionServer::executeGripperCommand(
  ...
  const auto kGoal = goal_handle->get_goal();
  const double kTargetWidth = 2 * kGoal->command.position;
  const double kCurrentWidth = gripper_->readOnce().width;

  bool grasp_action = (kTargetWidth != 0.0) && (kTargetWidth != 0.07);
  
  double ema_err = fabs(kCurrentWidth - kTargetWidth);
  double prev_ema = ema_err;
  ...
  while (!resultIsReady(result_future, future_wait_timeout_) && rclcpp::ok()) {
    ...
    if (grasp_action)
    {
      double err = fabs(gripper_->readOnce().width - kTargetWidth);
      ema_err = alpha * err + (1.0 - alpha) * ema_err;

      if (err <= 1e-3)
      {
        // if (kTargetWidth == 0.002)
        //   success = false;
        // else
          success = true;
      ...
      if (fabs(ema_err - prev_ema) <= 3e-3 && start_tol > stable)
        ++stable_count;
      else
        stable_count = 0;

      RCLCPP_INFO(get_logger(), "Value %f", fabs(ema_err - prev_ema));
      RCLCPP_INFO(get_logger(), "Count %d", stable_count);
      ...
      {
        success = true;
        RCLCPP_INFO(get_logger(), "New Width %f", gripper_->readOnce().width - 0.002);
        gripper_->grasp(gripper_->readOnce().width - 0.002, default_speed_, 30.0,
                           default_epsilon_inner_, default_epsilon_outer_);
      }
    }
  ...
  if (rclcpp::ok()) {
    ...
    kResult->effort = 0.;
    if (kResult->reached_goal || success) {
