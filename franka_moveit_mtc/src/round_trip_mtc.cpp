...
private:
    ...
    std::pair<double, double> second_pose_;
};
...
MTCTaskNode::MTCTaskNode(const rclcpp::NodeOptions& options)
    ...
    boost::split(_param_split, tmp_second_pose, boost::is_any_of(","));
    second_pose_ = {std::stod(_param_split[0]), std::stod(_param_split[1])};
...
void MTCTaskNode::roundTrip()
{
    ...
    current_obj_ = "object0";

    bool reverse{false};
    while(rclcpp::ok())
    {
        stage_failed_ = "";
        if(!setupObjectInformation(current_obj_))
            return;
        
        setupObjectPose(current_obj_);

        if (reverse)
        {
            target.pose.position.x = second_pose_.first;
            target.pose.position.y = second_pose_.second;
        }
        else {
            target.pose.position.x = first_pose_.first;
            target.pose.position.y = first_pose_.second;
        }
